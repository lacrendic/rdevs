  ...
  for(namevar in namevars){
    ...
    if(!missing(responses)){
      for(response in names(responses)){
        ...
        if(is.numeric(daux[[namevar]])){
          p <- p + stat_smooth(data=daux, aes_string(x=namevar,y=response), colour = color,method="loess",span=smooth_span, se = T,fill = "grey", alpha = 0.2)
        } else{
          p <- p +
...
plot_complete_df2 <- function (df, facet_formula, responses, smooth_span = 0.99, file = "output_plot_dist_by_var.pdf", ...) 
{
  require(ggplot2)
  require(scales)
  pdf(file, ...)
  ...
  for (namevar in namevars) {
    ...
    p <- ggplot(daux, aes_string(x = namevar)) + geom_bar(aes(y = (..count..)/sum(..count..)),fill="gray80") +
      xlab(NULL) + ylab(NULL) + ggtitle(str_capitalize(namevar)) + scale_y_continuous(labels = percent)
    if (!missing(responses)) {
      for (response in names(responses)) {
        color <- colors[which(response == names(responses))]
        if (is.numeric(daux[[namevar]])) {
          p <- p + stat_smooth(data = daux, aes_string(x = namevar, y = response), colour = color, method = "loess", 
                               span = smooth_span, se = T, fill = "gray", alpha = 0.05)
        }
        else {
          p <- p + stat_summary(data = daux, aes_string(x = namevar,  y = response), fun.y = mean, colour = color, 
                                geom = "point") + stat_summary(data = daux, aes_string(x = namevar, y = response, group = 1), 
                                                               fun.y = mean, colour = color, geom = "line")
        }
      }
    }
    if (!missing(facet_formula)) {
      p <- p + facet_grid(facet_formula, scales = "free") + 
        theme(text = element_text(size = 10), panel.grid = element_blank(), 
              panel.border = element_rect(color="grey80", size=0.5, linetype="solid",fill=NA), panel.background = element_blank(),
              legend.position = "bottom", legend.title = element_blank(),
              strip.text.x = element_text(size=9,colour="black"),
              strip.background = element_rect(colour="#D6D6D6", fill="#D6D6D6"),
              axis.text.x = element_text(angle = 45, hjust = 1,colour="black",size=4),
